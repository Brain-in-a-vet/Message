...
#targetUrl = "http://www.newseed.cn/invest/p1"
# 受资方    时间    轮次    金额    投资方
targetTitle="div.invest-list > table.table-list > tbody > tr"
...
# 对返回的列表进行遍历
with codecs.open("data_invest.txt",'a',encoding="gb2312") as f:
    for tr in data:
        isHead=tr.get('class');
        if isHead != None:
            continue
            ...
            if "td2" == td.get('class')[0]:
                for span in td.children:
                    if "td2-tit" == span.get('class')[0]:
                        trMsg+=span.a.get_text()+"#"
                    if "td2-com" == span.get('class')[0]:
                        trMsg+=span.span.get_text()+"#"
